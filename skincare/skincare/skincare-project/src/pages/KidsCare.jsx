...
const KidsCare = () => {
  ...
  // Get all kids care products
  const kidsCareProducts = products.filter(product => product.category === 'Kids Care')

  // Extract unique filter options
  const ageRanges = [...new Set(kidsCareProducts.map(product => product.ageRange))]
  const concerns = [...new Set(kidsCareProducts.flatMap(product => product.concerns || []))]
  const subcategories = [...new Set(kidsCareProducts.map(product => product.subcategory))]

  // Apply filters
  useEffect(() => {
    let result = kidsCareProducts
  ...
  return (
    <div className="px-3 py-8">
      <div className="mb-8 text-center">
        <h1 className="mb-4 text-4xl font-serif">Kids Care</h1>
        <p className="max-w-2xl mx-auto text-neutral-dark">
          Gentle, safe products specially formulated for children's delicate skin and hair.
          Our kids care range is dermatologist-tested and free from harsh chemicals.
        </p>
      </div>
      ...
      <div className="flex flex-col md:flex-row">
                ...
                {ageRanges.map(range => (
                  <label key={range} className="flex items-center">
                    <input
                      ...
                      onChange={() => toggleFilter('ageRange', range)}
                      className="mr-2"
                    />
                    {range}
                  </label>
        ...
        <div className="flex-1">
          <ProductGrid products={filteredProducts.length > 0 ? filteredProducts : kidsCareProducts} />
        </div>
      </div>
    </div>
  )
